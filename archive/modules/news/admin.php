...
<?php	
	...
	if($act=='editnews' || $act == 'editdraft')
	{
		...
		if(($newsParam = json_decode($newsStorage->get($prefix.$news))) != false){
			...
			<tr>
				<td>Идентификатор (исп. для URL):</td>
				<td><input type="text" name="id" id="id" value="'.($DUB == '1'?uniqid('n'):$news).'"><br><a href="javascript:void(0);" onclick="document.getElementById(\'id\').value = urlRusLat(document.getElementById(\'header\').value)">Сгенерировать из заголовка новости</a></td>
			</tr>';
