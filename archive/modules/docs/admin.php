...
'<div class="b">'+
'<button type="button" onclick="document.getElementById(\'inputimg\').value = \'/modules/docs/default.jpg\';closewindow(\'window\');">Вставить фото по умолчанию</button> '+
...
<?php
	...
	if($act=='info'){
		...
		'.$menu_page.'
		<div class="content">
			<p>Ваш RSS канал новостей находится по адресу <a href="/'.$docsConfig->idPage.'/rss.xml" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/rss.xml</a></p>
			...
			if($docsConfig->turbo){
				...
				if(($listIddocs = json_decode($docsStorage->get('list'), true)) != false){
					...
					for($i = 1; $i <= $countPage; ++$i){
						...
						<td><a href="//'.SERVER.'/'.$docsConfig->idPage.'/'.$docsConfig->turboId.'/'.$i.'.xml" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/'.$docsConfig->turboId.'/'.$i.'.xml</a></td>
						<td></td>
						</tr>';
	...
	if($act=='turbo1'){
		$docsConfig->turbo = 1;
		if($docsStorage->set('docsConfig', json_encode($docsConfig))){
...
<?php
	...
	if($act=='turbo_cfg'){
		...
		<tr>
			...
			<td><input type="text" name="turboItems" value="'.$docsConfig->turboItems.'"><br>
		...
		<tr>
			...
			<td><input type="text" name="turboId" value="'.$docsConfig->turboId.'"><br>
		...
		<tr>
			...
			<td>
				<select name="turboCacheTime">
					<option value="0"'.($docsConfig->turboCacheTime == '0'?' selected':'').'>Не кешировать (Не рекомендуется)
					<option value="900"'.($docsConfig->turboCacheTime == '900'?' selected':'').'>15 минут
					<option value="1800"'.($docsConfig->turboCacheTime == '1800'?' selected':'').'>30 минут
					<option value="3600"'.($docsConfig->turboCacheTime == '3600'?' selected':'').'>1 час (По умолчанию)
					<option value="10800"'.($docsConfig->turboCacheTime == '10800'?' selected':'').'>3 часа
					<option value="21600"'.($docsConfig->turboCacheTime == '21600'?' selected':'').'>6 часов
					<option value="43200"'.($docsConfig->turboCacheTime == '43200'?' selected':'').'>12 часов
					<option value="86400"'.($docsConfig->turboCacheTime == '86400'?' selected':'').'>1 день
					<option value="259200"'.($docsConfig->turboCacheTime == '259200'?' selected':'').'>3 дня
					<option value="604800"'.($docsConfig->turboCacheTime == '604800'?' selected':'').'>1 неделю
					<option value="1209600"'.($docsConfig->turboCacheTime == '1209600'?' selected':'').'>2 недели
					<option value="2678400"'.($docsConfig->turboCacheTime == '2678400'?' selected':'').'>1 месяц
					<option value="8035200"'.($docsConfig->turboCacheTime == '8035200'?' selected':'').'>3 месяца
					<option value="16070400"'.($docsConfig->turboCacheTime == '16070400'?' selected':'').'>6 месяцев
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td><textarea name="turboExceptions" style="height:150px;">'.$docsConfig->turboExceptions.'</textarea><br>
	...
	if($act=='turbo_addcfg'){
		...
		$docsConfig->turboItems = is_numeric($_POST['turboItems'])?$_POST['turboItems']:1000;
		$docsConfig->turboId = System::validPath($_POST['turboId'])?$_POST['turboId']:'turbo';
		$docsConfig->turboCacheTime = is_numeric($_POST['turboCacheTime'])?$_POST['turboCacheTime']:3600;
		$docsConfig->turboExceptions = htmlspecialchars($_POST['turboExceptions']);
...
<?php
	...
	if($act=='index')
	{
		if(isset($_GET['dub'])){
			if($_GET['dub'] == '1'){
				$docs = htmlspecialchars(specfilter($_GET['docs']));
				if(($docsParam = json_decode($docsStorage->get('docs_'.$docs))) != false){
		...
		<input type="hidden" name="act" id="act" value="adddocs">
		<table class="tblform">
		<tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="img" id="inputimg" value="/modules/docs/default.jpg"> 
				<button type="button" onClick="openwindow(\'window\', 750, \'auto\', iframefiles);">Выбрать файл</button>
			</td>
		</tr>
		<tr>
			<td></td>
			<td><img src="/modules/docs/default.jpg" alt="" id="img" style="width: 380px;"></td>
		</tr>
		<tr>
		...
		<td>
			...
			foreach($docsConfig->cat as $key => $value){
				echo'<option value="'.$key.'"'.($docsParam->cat == $key?' selected':'').'>'.$value;
			}
			echo'</select><br>
...
<?php
	...
	if($act=='adddocs' || $act=='adddraft'){
		...
		// 5.1.14
		$param['time'] = time();
		$param['date'] = date($docsConfig->formatDate, $param['time']);
		// 5.1.18
		...
		$docs = htmlspecialchars(specfilter($_POST['docs']));

		if($act=='adddocs')
		{
			$id = ($docsStorage->iss('docs_'.$_POST['id']) == false && System::validPath($_POST['id']))?$_POST['id']:uniqid('n');
			if($docsStorage->set('docs_'.$id, json_encode($param))){
				// Добавляем ID новости в список
				$listIddocs = json_decode($docsStorage->get('list'), true); // Получили список ввиде массива
				$listIddocs[] = $id; // Добавили новый элемент массива в конец
				$docsStorage->set('list', json_encode($listIddocs)); // Записали массив в виде json
				// Добавляем в категории 
				$listIdCat = json_decode($docsStorage->get('category'), true); // Получили список ввиде массива
				$listIdCat[$id] = $param['cat']; // Добавили новый элемент массива в конец
				$docsStorage->set('category', json_encode($listIdCat)); // Записали массив в виде json
			...
			if($docsStorage->delete('draft_'.$docs)){
				// Удаляем страницу из черновиков
				$draftListIddocs = json_decode($docsStorage->get('draftList'), true); // Получили список ввиде массива
				if(($key = array_search($docs, $draftListIddocs)) !== false){
					unset($draftListIddocs[$key]); // Удалили найденый элемент массива
				}
				$draftListIddocs = array_values($draftListIddocs); // Переиндексировали числовые индексы 
				$docsStorage->set('draftList', json_encode($draftListIddocs)); // Записали массив в виде json
				System::notification('Удалена новость из черновика с идентификатором '.$docs.'', 'g');
			}
		}

		if($act=='adddraft'){
			$id = $docsStorage->iss('draft_'.$_POST['id']) == false && System::validPath($_POST['id'])?$_POST['id']:uniqid('n');
			if($docsStorage->set('draft_'.$id, json_encode($param))){
				// Добавляем ID новости в список
				$draftListIddocs = json_decode($docsStorage->get('draftList'), true); // Получили список ввиде массива
				$draftListIddocs[] = $id;// Добавили новый элемент массива в конец
				$docsStorage->set('draftList', json_encode($draftListIddocs)); // Записали массив в виде json
...
<?php
	...
	if($act=='cfg'){
		...
		$checked = ($docsConfig->commentEngine == 1)?'checked':'';
		...
		<tr>
			...
			<td><input type="text" name="navigation" value="'.$docsConfig->navigation.'" maxlength="3"></td>
		</tr>

		<tr>
			<td>Количество превью при выводе в блоке:</td>
			<td><input type="text" name="countInBlok" value="'.$docsConfig->countInBlok.'" maxlength="3"></td>
		</tr>
		
		<tr>
			<td>Формат вывода даты (Формат функции date):</td>
			<td><input type="text" name="formatDate" value="'.$docsConfig->formatDate.'"></td>
		</tr>
		
		<tr>
			<td>Идентификатор страницы с новостями:</td>
			<td><input type="text" name="idPage" value="'.$docsConfig->idPage.'"></td>
		</tr>
		
		<tr>
			<td>Идентификатор страницы пользователей:</td>
			<td><input type="text" name="idUser" value="'.$docsConfig->idUser.'"></td>
		</tr>
		...
		<tr>
			...
			<td class="middle">'.(file_exists(Module::pathRun($Config->template, 'docs.content.template'))?'<a class="link" target="_blank" href="files.php?act=editor&amp;dir=../modules/'.$Config->template.'&file=../modules/'.$Config->template.'/docs.content.template.php">Открыть редактор для правки шаблона</a>':'<span class="comment">Не предусмотрен</span>').'</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				foreach($docsConfig->cat as $key => $value){
					echo'<option value="'.$key.'"'.($docsConfig->indexCat === $key?' selected':'').'>'.$value;
				}
				echo'<option value="-1"'.($docsConfig->indexCat == '-1'?' selected':'').'>Не выводить новости на начальной странице
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<option value="0">Выводить из всех категорий';
				foreach($docsConfig->cat as $key => $value){
					echo'<option value="'.$key.'"'.($docsConfig->blokCat === $key?' selected':'').'>'.$value;
				}
				echo'</select>
			</td>
		</tr>
		...
		<tr id="trCommentTemplate">
			<td class="top">Код сервиса комментариев:<br><span class="comment">Подробнее о сервисах комментариев <a href="http://my-engine.ru/docscomments">тут</a></span></td>
			<td><TEXTAREA NAME="commentTemplate" id="textareaCommentTemplate" ROWS="20" COLS="100" style="height:150px;">'.htmlspecialchars($docsConfig->commentTemplate).'</TEXTAREA></td>
		</tr>
	...
	if($act=='addcfg'){
		...
		if( !is_numeric($_POST['navigation']) || 
			...
			$docsConfig->navigation = htmlspecialchars(specfilter($_POST['navigation']));
			$docsConfig->countInBlok = htmlspecialchars(specfilter($_POST['countInBlok']));
			$docsConfig->formatDate = htmlspecialchars(specfilter($_POST['formatDate']));
			$docsConfig->idPage = htmlspecialchars(specfilter($_POST['idPage']));
			$docsConfig->idUser = htmlspecialchars(specfilter($_POST['idUser']));
			// $docsConfig->prevTemplate = $_POST['prevTemplate'];
			$docsConfig->contentTemplate = $_POST['contentTemplate'];
			$docsConfig->commentTemplate = $_POST['commentTemplate'];
			$docsConfig->commentEngine = ($_POST['commentEngine'] == 'y')?'1':'0';
			$docsConfig->blokCat = htmlspecialchars(specfilter($_POST['blokCat']));
			$docsConfig->indexCat = htmlspecialchars(specfilter($_POST['indexCat']));
...
<?php	
	...
	if($act=='edit')
	{
		...
		if(isset($_GET['nom_page']) == false || $_GET['nom_page'] == 1 || $_GET['nom_page'] == ''){
			...
			if(($draftListIddocs = json_decode($docsStorage->get('draftList'), true)) == false){
				...
				$draftListIddocs = array_reverse($draftListIddocs);

				foreach($draftListIddocs as $value){
					if($docsStorage->iss('draft_'.$value)){
						$docsParam = json_decode($docsStorage->get('draft_'.$value));
							
						$comments = ($docsParam->comments == '1')?'<span style="color: green;">Включено</span>':'<span style="color: red;">Выключено</span>';
						
						echo'<div class="item item_page">
						<div class="right_menu">
						<a href="module.php?module='.$MODULE.'&amp;act=editdraft&amp;docs='.$value.'&amp;dub=1">Создать дубликат</a>
						<a href="module.php?module='.$MODULE.'&amp;act=editdraft&amp;docs='.$value.'">Редактировать</a>
						<a href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', dell(\'module.php?module='.$MODULE.'&amp;act=delldraft&amp;docs='.$value.'&amp;nom_page=1\', \''.$docsParam->header.'\', \''.SERVER.'/'.$docsConfig->idPage.'/'.$value.'\'));">Удалить</a>
						</div>
						<div class="name_page"><img src="include/page.svg" alt=""> <a href="module.php?module='.$MODULE.'&amp;act=editdraft&amp;docs='.$value.'&amp;nom_page=1">'.$docsParam->header.'</a></div>
						<div>URL: <a href="//'.SERVER.'/'.$docsConfig->idPage.'/'.$value.'" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/'.$value.'</a></div>
						<div>Дата редактирования: '.(isset($docsParam->time)?date($docsConfig->formatDate, $docsParam->time):$docsParam->date).'</div>
						</div>';
					}else{
						echo'<div class="item item_page">
						...
						<div>Index: '.$value.'</div>
						<div class="right_menu">
						<a href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', dell(\'module.php?module='.$MODULE.'&amp;act=delldraft&amp;docs='.$value.'\', \''.$docsParam->header.'\', \''.SERVER.'/'.$docsConfig->idPage.'/'.$value.'\'));">Удалить</a>
						</div>
						</div>';
					}
				}
			...
			while($i < $var){
				if($i < $nom){
					if($docsStorage->iss('docs_'.$listIddocs[$i])){
						...
						// <td><a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$listIddocs[$i].'&amp;nom_page='.$nom_page.'">'.$docsParam->header.'</a></td>
						// <td><a href="//'.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'</a></td>
						// <td>'.$comments.'</td>
						// <td>'.(isset($docsParam->time)?date($docsConfig->formatDate, $docsParam->time):$docsParam->date).'</td>
						// <td><a href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', dell(\'module.php?module='.$MODULE.'&amp;act=dell&amp;docs='.$listIddocs[$i].'&amp;nom_page='.$nom_page.'\', \''.$docsParam->header.'\', \''.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'\'));">Удалить</a></td>
						// </tr>';

						echo'<div class="item item_page">
						<div class="right_menu">
						<a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$listIddocs[$i].'&amp;dub=1">Создать дубликат</a>
						<a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$listIddocs[$i].'&amp;nom_page='.$nom_page.'">Редактировать</a>
						<a href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', dell(\'module.php?module='.$MODULE.'&amp;act=dell&amp;docs='.$listIddocs[$i].'&amp;nom_page='.$nom_page.'\', \''.$docsParam->header.'\', \''.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'\'));">Удалить</a>
						</div>
						<div class="name_page"><img src="include/page.svg" alt=""> <a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$listIddocs[$i].'&amp;nom_page='.$nom_page.'">'.$docsParam->header.'</a></div>
						<div>URL: <a href="//'.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'</a></div>
						<div>Дата публикации: '.(isset($docsParam->time)?date($docsConfig->formatDate, $docsParam->time):$docsParam->date).'</div>
						</div>';
					}else{
						echo'<div class="item item_page">
						<div style="color: red;">Error: индекс не связан ни с одной страницей</div>
						<div>Index: '.$listIddocs[$i].'</div>
						<div class="right_menu">
						<a href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', dell(\'module.php?module='.$MODULE.'&amp;act=dell&amp;docs='.$listIddocs[$i].'&amp;nom_page='.$nom_page.'\', \''.$docsParam->header.'\', \''.SERVER.'/'.$docsConfig->idPage.'/'.$listIddocs[$i].'\'));">Удалить</a>
						</div>
						</div>';
	...
	if($act=='search'){
		if(!function_exists('mb_stripos')){
			...
			if(($listIddocs = json_decode($docsStorage->get('list'), true)) == false){
				...
				$listIddocs = array_reverse($listIddocs);//перевернули масив
				...
				foreach($listIddocs as $value){
					if($docsStorage->iss('docs_'.$value)){
						$docsParam = json_decode($docsStorage->get('docs_'.$value));

						if(mb_stripos($docsParam->header, $q, 0, 'UTF-8') !== false){
							$pSearchName[] = array('header' => $docsParam->header, 'time' => $docsParam->time, 'id' => $value);
						}elseif(mb_stripos($docsParam->title, $q, 0, 'UTF-8') !== false){
							$pSearchTitle[] = array('header' => $docsParam->header, 'time' => $docsParam->time, 'id' => $value);
						}elseif(mb_stripos($docsParam->keywords, $q, 0, 'UTF-8') !== false){
							$pSearchKeywords[] = array('header' => $docsParam->header, 'time' => $docsParam->time, 'id' => $value);
						}elseif(mb_stripos($docsParam->description, $q, 0, 'UTF-8') !== false){
							$pSearchDescription[] = array('header' => $docsParam->header, 'time' => $docsParam->time, 'id' => $value);
						}elseif(mb_stripos($value, $q, 0, 'UTF-8') !== false){
							$pSearchID[] = array('header' => $docsParam->header, 'time' => $docsParam->time, 'id' => $value);
						}
				...
				foreach($pSearchResult as $value){
					...
					echo'<div class="item item_page">
					<div class="right_menu">
					<a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$value['id'].'&amp;dub=1">Создать дубликат</a>
					<a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$value['id'].'&amp;">Редактировать</a>
					<a href="javascript:void(0);" onclick="openwindow(\'window\', 650, \'auto\', dell(\'module.php?module='.$MODULE.'&amp;act=dell&amp;docs='.$value['id'].'&amp;\', \''.$value['header'].'\', \''.SERVER.'/'.$docsConfig->idPage.'/'.$value['id'].'\'));">Удалить</a>
					</div>
					<div class="name_page"><img src="include/page.svg" alt=""> <a href="module.php?module='.$MODULE.'&amp;act=editdocs&amp;docs='.$value['id'].'&amp;">'.preg_replace('#'.$q.'#ius', '<span class="r">'.$q.'</span>', $value['header']).'</a></div>
					<div>URL: <a href="//'.SERVER.'/'.$docsConfig->idPage.'/'.$value['id'].'" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/'.$value['id'].'</a></div>
					<div>Дата публикации: '.(isset($value['time'])?date($docsConfig->formatDate, $value['time']):$value['date']).'</div>
					</div>';
	...
	if($act=='dell' || $act == 'delldraft')
	{
		...
		$prefix = ($act == 'dell')?'docs_':'draft_';
		if($docsStorage->delete($prefix.$docs)){ // Удадляем новость
			if($act=='dell'){
				//Удаляем страницу из списка
				$listIddocs = json_decode($docsStorage->get('list'), true); // Получили список ввиде массива
				if(($key = array_search($docs, $listIddocs)) !== false){
					unset($listIddocs[$key]); // Удалили найденый элемент массива
				}
				$listIddocs = array_values($listIddocs); // Переиндексировали числовые индексы 
				$docsStorage->set('list', json_encode($listIddocs)); // Записали массив в виде json

				// Удаляем страницу из категорий
				$listIdCat = json_decode($docsStorage->get('category'), true); // Получили список ввиде массива
				unset($listIdCat[$docs]);
				$docsStorage->set('category', json_encode($listIdCat)); // Записали массив в виде json
				
				$docsStorage->delete('comments_'.$docs); // Удаляем комментарии
				$docsStorage->delete('count_'.$docs); // Удаляем счетчик комментариев
				
				System::notification('Удалена новость с идентификатором '.$docs.'', 'g');
			...
			System::notification('Ошибка при удалении новости с идентификатором '.$docs.', страница не найдена или запрос некорректен', 'r');
...
<?php	
	...
	if($act=='editdocs' || $act == 'editdraft')
	{
		$docs = htmlspecialchars(specfilter($_GET['docs']));
		$nom_page = isset($_GET['nom_page'])?htmlspecialchars(specfilter($_GET['nom_page'])):1;
		$prefix = ($act == 'editdocs')?'docs_':'draft_';
		$DUB = isset($_GET['dub'])?$_GET['dub']:0;

		if(($docsParam = json_decode($docsStorage->get($prefix.$docs))) != false){
			...
			<div class="content">
			<form name="form_name" action="module.php?module='.$MODULE.'&amp;" method="post" style="margin:0px; padding:0px;">
			<INPUT TYPE="hidden" NAME="act" id="act" VALUE="'.($DUB == '1'?'adddocs':'addedit').'">
			...
			<input type="hidden" name="time" value="'.(isset($docsParam->time)?$docsParam->time:strtotime($docsParam->date)).'">
			
			<table class="tblform">
			<tr>
				<td>Заголовок новости:</td>
				<td><input type="text" name="header" id="header" value="'.$docsParam->header.'"></td>
			</tr>
			
			<tr>
				<td class="top">Превью новости:</td>
				<td><TEXTAREA NAME="prev" ROWS="20" COLS="100" style="height: 150px;">'.htmlspecialchars($docsParam->prev).'</TEXTAREA></td>
			</tr>
			<tr>
				<td class="top">Содержимое новости:</td>
				<td><TEXTAREA NAME="content" ROWS="20" COLS="100" style="height: 250px;">'.htmlspecialchars($docsParam->content).'</TEXTAREA></td>
			</tr>';
			$checked = ($docsParam->comments == 1)?'checked':'';
			echo'
			<tr>
				<td>Разрешить комментирование</td>
			...
			<tr>
				<td>URL иллюстр. картинки:</td>
				<td>
					<input type="text" name="img" id="inputimg" value="'.$docsParam->img.'"> 
					<button type="button" onClick="openwindow(\'window\', 700, \'auto\', iframefiles);">Выбрать файл</button>
				</td>
			</tr>
			<tr>
				<td></td>
				<td><img src="'.$docsParam->img.'" alt="" id="img" style="width: 380px;"></td>
			</tr>
			<tr>
				<td>Ключевые слова (keywords):</td>
				<td><input type="text" name="keywords" value="'.$docsParam->keywords.'"></td>
			</tr>

			<tr>
				<td>Описание (description):</td>
				<td><input type="text" name="description" value="'.$docsParam->description.'"></td>
			</tr>
			...
			<tr>
				<td>Идентификатор (исп. для URL):</td>
				<td><input type="text" name="id" id="id" value="'.($DUB == '1'?uniqid('n'):$docs).'"><br><a href="javascript:void(0);" onclick="document.getElementById(\'id\').value = urlRusLat(document.getElementById(\'header\').value)">Сгенерировать из заголовка новости</a></td>
			</tr>';
			...
			foreach($docsConfig->custom as $value){
				echo'<tr>
					<td>'.$value->name.':</td>
					<td>
						'.($value->type == 'input'?'<input type="text" name="custom['.$value->id.']" value="'.(isset($docsParam->custom->{$value->id})?htmlspecialchars($docsParam->custom->{$value->id}):'').'">':'').'
						'.($value->type == 'textarea'?'<textarea name="custom['.$value->id.']"  style="height:150px;">'.(isset($docsParam->custom->{$value->id})?htmlspecialchars($docsParam->custom->{$value->id}):'').'</textarea>':'').'
					</td>
				</tr>';
			}
			if($act == 'editdocs'){
			...
			if($act == 'editdraft'){
				if ($docsStorage->iss('docs_'.$docs)){
					echo'<tr>
						<td>&nbsp;</td>
						<td><button type="button" onClick="document.getElementById(\'act\').value = \'adddocs\'; submit();" title="Опубликовать как новую новость">Опубликовать</button> &nbsp; <a href="module.php?module='.$MODULE.'&amp;act=edit&amp;nom_page='.$nom_page.'">Вернуться назад</a></td>
					</tr>';
					echo'<tr>
						<td>&nbsp;</td>
					...
					echo'<tr>
						<td>&nbsp;</td>
						<td><button type="button" onClick="document.getElementById(\'act\').value = \'adddocs\'; submit();">Опубликовать</button> &nbsp; <a href="module.php?module='.$MODULE.'&amp;act=edit&amp;nom_page='.$nom_page.'">Вернуться назад</a></td>
					</tr>';
				}
...
<?php
	...
	if($act == 'addedit' || $act == 'addeditdraft'){
		...
		$id_docs = htmlspecialchars(specfilter($_POST['id'])); // Новый id для новости
		$cat = htmlspecialchars(specfilter($_POST['cat'])); // Новый id для новости
		$prefix = ($act == 'addedit')?'docs_':'draft_';
		
			if(($docsParam = json_decode($docsStorage->get($prefix.$docs))) != false){
				
				$docsParam->header = ($_POST['header'] == '')?'Без названия':htmlspecialchars(specfilter($_POST['header']));
				$docsParam->img = htmlspecialchars(specfilter($_POST['img']));
				$docsParam->keywords = htmlspecialchars(specfilter($_POST['keywords']));
				$docsParam->description = htmlspecialchars(specfilter($_POST['description']));
				$docsParam->prev = $_POST['prev'];
				$docsParam->content = $_POST['content'];
				//$param['date'] = htmlspecialchars(specfilter($_POST['date'])); // удалено в 5.1.14
				$docsParam->comments = (isset($_POST['comments']) && $_POST['comments'] == 'y')?'1':'0';
				// 5.1.14
				if(!isset($docsParam->time)){
					$docsParam->time = strtotime($docsParam->date);
					$docsParam->date = date($docsConfig->formatDate, $docsParam->time);
				}
				// 5.1.18
				$newCat = $docsParam->cat != $cat?true:false;
				$docsParam->cat = $cat;
				// 5.1.20
				$array = array();
				if(isset($_POST['custom'])){
				...
				$docsParam->custom = $array;
				// 5.1.25
				if($act == 'addeditdraft'){$docsParam->time = time();} // обновляемая дата для черновика
				//


				
				if($docsStorage->set($prefix.$docs, json_encode($docsParam))){
					if($act == 'addedit'){
						if($newCat){
							// Замена категории
							$listIdCat = json_decode($docsStorage->get('category'), true); // Получили список ввиде массива
							$listIdCat[$docs] = $docsParam->cat;
							$docsStorage->set('category', json_encode($listIdCat)); // Записали массив в виде json
						}
						// Удаление новости из черновика при публикации
						if($docsStorage->delete('draft_'.$docs)){
					...
					if($id_docs != $docs){
						if($docsStorage->iss($prefix.$id_docs) == false && System::validPath($id_docs)){
							
							if($docsStorage->set($prefix.$id_docs, json_encode($docsParam)) == false){
								System::notification('Ошибка при записи ключа '.$prefix.$id_docs.'', 'r');
							}

							if($docsStorage->delete($prefix.$docs) == false){
								System::notification('Ошибка при удалении ненужного ключа '.$prefix.$docs.'', 'r');
							}
							...
							if($act == 'addedit'){
								...
								$listIddocs = json_decode($docsStorage->get('list'), true); // Получили список ввиде массива
								if(($key = array_search($docs, $listIddocs)) !== false){
									$listIddocs[$key] = $id_docs; // Заменили найденый элемент массива
								}
								$listIddocs = array_values($listIddocs); // Переиндексировали числовые индексы 
								$docsStorage->set('list', json_encode($listIddocs)); // Записали массив в виде json
								...
								foreach($listIdCat as $key => $value){
									if($docs == $key){
										$newArr[$id_docs] = $value;
									}else{
										$newArr[$key] = $value;
									}
								}
								$docsStorage->set('category', json_encode($newArr)); // Записали массив в виде json
							}
							...
							if($act == 'addeditdraft'){
								...
								$draftListIddocs = json_decode($docsStorage->get('draftList'), true); // Получили список ввиде массива
								if(($key = array_search($docs, $draftListIddocs)) !== false){
									$draftListIddocs[$key] = $id_docs; // Заменили найденый элемент массива
								}
								$draftListIddocs = array_values($draftListIddocs); // Переиндексировали числовые индексы 
								$docsStorage->set('draftList', json_encode($draftListIddocs)); // Записали массив в виде json
							}

							System::notification('Отредактирована новость со сменой идентификатора '.$docs.' на идентификатор '.$id_docs.', ссылка на страницу '.$Config->protocol.'://'.SERVER.'/'.$docsConfig->idPage.'/'.$id_docs, 'g');
							echo'<div class="msg">Новость успешно сохранена</div>';
						}else{
							System::notification('Отредактирована новость с неудачной попыткой смены идентификатора '.$docs.' на идентификатор '.$id_docs.', идентификатор '.$id_docs.' уже существует или некорректен, ссылка на страницу '.$Config->protocol.'://'.SERVER.'/'.$docsConfig->idPage.'/'.$docs, 'g');
							echo'<div class="msg">Новость сохранена но идентификатор изменить не удалось</div>';
						}
					}else{
						System::notification('Отредактирована новость с идентификатором '.$id_docs.', ссылка на страницу '.$Config->protocol.'://'.SERVER.'/'.$docsConfig->idPage.'/'.$id_docs, 'g');
						echo'<div class="msg">Новость успешно сохранена</div>';
					}
				}else{
					System::notification('Ошибка при сохранении страницы с идентификатором '.$docs.', ошибка записи', 'r');
					echo'<div class="msg">Ошибка при сохранении страницы</div>';
					
				}
			}else{
				System::notification('Ошибка при сохранении страницы с идентификатором '.$docs.', страница ненайдена', 'r');
...
<?php
	...
	if($act=='comment')
	{
		...
		if ($docsConfig->commentEngine){
			...
			if(($lastComments = json_decode($docsStorage->get('lastComments'), true)) == false){
				...
				while($i < $var){
					if($i < $nom){
						...
						echo'<div class="item">
							<div><INPUT TYPE="checkbox" NAME="comment[]" VALUE="'.$lastComments[$i]['idComment'].'"> '.($lastComments[$i]['published']?'':'<span class="r">Не опубликованно</span>').' Страница: <a href="//'.SERVER.'/'.$docsConfig->idPage.'/'.$lastComments[$i]['iddocs'].'" target="_blank">'.SERVER.'/'.$docsConfig->idPage.'/'.$lastComments[$i]['iddocs'].'</a></div>
							<h3><img src="include/user.svg" alt=""> '.$lastComments[$i]['login'].' </h3>
							'.docsFormatText($lastComments[$i]['text']).'
	...
	if($act=='pubcoment'){
		...
		if(($lastComments = json_decode($docsStorage->get('lastComments'), true)) == false){
			...
			foreach($addComment as $key => $value){
				$arrayComments = json_decode($docsStorage->get('comments_'.$key), true);
				...
				if($docsStorage->set('comments_'.$key, json_encode($arrayComments))){
					
					$count = $docsStorage->iss('count_'.$key)?$docsStorage->get('count_'.$key):0;
					$count+= $countPP;
					$docsStorage->set('count_'.$key, $count);
		...
		if(($lastComments = json_decode($docsStorage->get('lastComments'), true)) == false){
			...
			foreach($lastComments as $key => $value){
				if(in_array($value['idComment'], $_POST['comment'])){
					$dellComment[$value['iddocs']][] = $value['idComment'];
			...
			foreach($dellComment as $key => $value){
				...
				$docsStorage->set('comments_'.$key, json_encode($arrayComments));
			}
...
<?php	
	...
	if($act=='listdellcoment'){
		$docsStorage->set('lastComments', json_encode(array()));
...
<?php	
	...
	if($act=='cfgcomment'){
		...
		<tr>
			...
			<td>
				<SELECT NAME="commentEnable" >
					<OPTION VALUE="0" '.($docsConfig->commentEnable == '0'?'selected':'').'>Выключено
					<OPTION VALUE="1" '.($docsConfig->commentEnable == '1'?'selected':'').'>Включено
		...
		<tr>
			...
			<td>
				<SELECT NAME="commentRules" >
					<OPTION VALUE="0" '.($docsConfig->commentRules == '0'?'selected':'').'>Все пользователи
					<OPTION VALUE="1" '.($docsConfig->commentRules == '1'?'selected':'').'>Только зарегистрированные пользователи
					<OPTION VALUE="2" '.($docsConfig->commentRules == '2'?'selected':'').'>Только пользователи с преференциями
					<OPTION VALUE="3" '.($docsConfig->commentRules == '3'?'selected':'').'>Только администратор
				</SELECT>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<SELECT NAME="commentModeration" >
					<OPTION VALUE="0" '.($docsConfig->commentModeration == '0'?'selected':'').'>Не модерировать, публиковать сразу
					<OPTION VALUE="1" '.($docsConfig->commentModeration == '1'?'selected':'').'>Модерировать не зарегистрированных пользователей и новичков
					<OPTION VALUE="2" '.($docsConfig->commentModeration == '2'?'selected':'').'>Модерировать всех кроме пользователей с преференциями
				</SELECT>
			</td>
		</tr>
		...
		<tr>
			...
			<td><input type="text" name="commentModerationNumPost" value="'.$docsConfig->commentModerationNumPost.'">
		...
		<tr>
			...
			<td><input type="text" name="commentMaxLength" value="'.$docsConfig->commentMaxLength.'"></td>
		</tr>
		
		<tr>
			<td>Кол-во выводимых комментариев за раз:</td>
			<td><input type="text" name="commentNavigation" value="'.$docsConfig->commentNavigation.'"></td>
		</tr>
		
		<tr>
			<td>Макс. комментариев для одной новости:</td>
			<td><input type="text" name="commentMaxCount" value="'.$docsConfig->commentMaxCount.'"></td>
		</tr>
		
		<tr>
			<td>Задержка на проверку новых комментарий:</td>
			<td><input type="text" name="commentCheckInterval" value="'.$docsConfig->commentCheckInterval.'">
	...
	if($act=='addcfgcomment'){
		...
		if( !is_numeric($_POST['commentEnable'])||
			...
			$docsConfig->commentEnable = $_POST['commentEnable'];
			$docsConfig->commentRules = $_POST['commentRules'];
			$docsConfig->commentModeration = $_POST['commentModeration'];
			$docsConfig->commentModerationNumPost = $_POST['commentModerationNumPost'];
			$docsConfig->commentMaxLength = $_POST['commentMaxLength'];
			$docsConfig->commentNavigation = $_POST['commentNavigation'];
			$docsConfig->commentMaxCount = $_POST['commentMaxCount'];
			$docsConfig->commentCheckInterval = $_POST['commentCheckInterval'];
