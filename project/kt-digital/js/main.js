...
var preloader = document.querySelector('.preloader__wrapper');
    ...
    function animOnScroll() {
      for (var index = 0; index < animItems.length; index++) {
        ...
        if (pageYOffset > animItemOffSet - animItemPoint && pageYOffset < animItemOffSet + animItemHeight) {
          ...
          if (!animItem.classList.contains("anim__no__hide")) {
...
var menu__link = document.querySelectorAll('.header__nav li.nav__item a');
var overflowHidden = document.querySelector('body');
...
var hideMenu = function hideMenu(event) {
  ...
  setTimeout(function () {
    ...
    menu.classList.remove("anim__active");
...
menuBtn.addEventListener('click', function (event) {
  ...
  if (!menu.classList.contains('menu__active')) {
    ...
    setTimeout(function () {
      menu.classList.add("anim__active");
    }, 300);
  } else {
    menu.classList.add('menu__hide');
    menuBtn.classList.remove('menu__active');
...
function check_radio_selected(elem_id) {
  obj = document.querySelector('.' + elem_id + ' label.checked');

  if (!obj) {
    $('.warning__input[data-name="' + elem_id + '"]').each(function () {
...
(function ($) {
  ...
  $("#to_step1").click(function (event) {
    ...
    if (check_input_value("your__niche")) {
      to_step(1, true);
      quizValid = true;
    }
  });
  $("#to_step2").click(function (event) {
    ...
    if (check_input_value("other__quiz") || check_radio_selected("checkbox-2")) {
      to_step(2, true);
    }
  });
  $("#to_step3").click(function (event) {
    ...
    if (check_radio_selected("checkbox-3")) {
      to_step(3, true);
    }
  });
  $("#to_step33").click(function (event) {
    event.preventDefault();
    to_step(3, true);
  });
  $("#to_step4").click(function (event) {
    ...
    if (check_radio_selected("checkbox-4")) {
      to_step(4, true);
    }
  });
  $("#to_step5").click(function (event) {
    ...
    if (check_input_value("your__name__quiz") && check_input_value("your__number__quiz")) {
      to_step(5, true);
    }
  });
  $("#to_step6").click(function (event) {
    ...
    if (check_radio_selected("qsi__wrapp")) {
      $("#quiz_form").submit();
    }
  });
  $('#quiz_form').submit(function (event) {
    event.preventDefault();
    quizValid = false; // Шаг 1/6

    var niche = $(this).find("input[name='your__niche']").val();
    var site__link;

    if ($(this).find("input[name='your__site__link']").val() == "") {
      var site__link = "Нет";
    } else {
      site__link = $(this).find("input[name='your__site__link']").val();
    }

    var social__link;

    if ($(this).find("input[name='your__social__link']").val() == "") {
      var social__link = "Нет";
    } else {
      social__link = $(this).find("input[name='your__social__link']").val();
    } // Шаг 2/6


    var elems1 = document.querySelectorAll(".checkbox-2 label.checked input[type='checkbox']");
    var checkbox__2 = [].map.call(elems1, function (obj) {
      return obj.value;
    });
    var checkbox__2__val = checkbox__2.join();

    if ($(this).find("input[name='other__quiz']").val() == "") {
      var other__quiz = "Нет";
    } else {
      other__quiz = $(this).find("input[name='other__quiz']").val();
    } // Шаг 3/6


    var elems2 = document.querySelectorAll(".checkbox-3 label.checked input[type='checkbox']");
    var checkbox__3 = [].map.call(elems2, function (obj) {
      return obj.value;
    });
    var checkbox__3__val = checkbox__3.join(); // Шаг 4/6

    var elems3 = document.querySelectorAll(".checkbox-4 label.checked input[type='checkbox']");
    var checkbox__4 = [].map.call(elems3, function (obj) {
      return obj.value;
    });
    var checkbox__4__val = checkbox__4.join(); // Шаг 5/6

    var name = $(this).find("input[name='your__name__quiz']").val();
    var number = $(this).find("input[name='your__number__quiz']").val(); // Шаг 6/6

    var checkbox__5__val = $(this).find(".quiz__switch__item label.checked input[type='checkbox']").val();
    $.ajax({
      type: 'POST',
      async: true,
      cache: false,
      data: {
        ...
        // Шаг 2/6
        checkbox__2__val: checkbox__2__val,
        other__quiz: other__quiz,
        // Шаг 3/6
        checkbox__3__val: checkbox__3__val,
        // Шаг 4/6
        checkbox__4__val: checkbox__4__val,
        // Шаг 5/6
        name: name,
        number: number,
        // Шаг 6/6
        checkbox__5__val: checkbox__5__val
      },
      url: "/wp-admin/admin-ajax.php",
      success: function success(data) {
        $(this).html(data);
        to_step(6, true);
        ym(90018441, 'reachGoal', 'quiz');
      }
    });
    return false;
...
var wpcf7Elm = document.querySelector('.modal__wrapp[data-target="price"] .modal__content');
wpcf7Elm.addEventListener('wpcf7mailsent', function (event) {
  event.stopPropagation();
  var FormOff = $(this).find('.price__form');
  var MessageOff = $(this).find('.wpcf7-response-output');
  var successMessageOn = $(this).find('.success');

  if (successMessage == false) {
    FormOff.css('display', 'none');
    MessageOff.css('display', 'none');
    successMessageOn.css('display', 'flex');
    successMessage = true;
    $('.checkbox__default').css('display', 'flex');
  }
}, false);
document.addEventListener('wpcf7mailsent', function (event) {
  ...
  if (successMessage == false) {
    event.target.querySelector('.form__wrapp').style.display = 'none';
    event.target.querySelector('.wpcf7-response-output').style.display = 'none';
    event.target.querySelector('.success').style.display = 'flex';
    successMessage = true;
    $('.checkbox__default').css('display', 'flex');
  }
}, false);
    ...
    value: function close() {
      if (this.modalContainer) {
        ...
        if (successMessage == true) {
          $('.form__wrapp').css('display', 'block');
          $('.price__form').css('display', 'block');
          $('.wpcf7-response-output').css('display', 'none');
          $('.success').css('display', 'none');
          successMessage = false;
        }

        if (bgModalPortfolio == true) {
          $('.modal__wrapp[data-target="zayavka"]').find('.order__call').css('background', 'rgba(255, 255, 255, 0.08)');
          bgModalPortfolio = false;
        }
...
/* data-input-value="Сайты START" */

var submitForm__site = false;
var submitForm__smm = false;
var submitForm__brand = false;
var submitForm__crm = false;
var servicesBtn = document.querySelectorAll('.btn__services');
servicesBtn.forEach(function (inputValueService) {
  return inputValueService.addEventListener('click', function () {
    var serviceValue = inputValueService.getAttribute('data-input-value');
    document.querySelector('input.service__input').value = serviceValue;

    if (serviceValue == "Портфолио" && bgModalPortfolio == false) {
      $('.modal__wrapp[data-target="zayavka"]').find('.order__call').css('background', '#2C3F40');
      bgModalPortfolio = true;
    }
  });
});
...
swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"].use([swiper_bundle__WEBPACK_IMPORTED_MODULE_0__.Navigation, swiper_bundle__WEBPACK_IMPORTED_MODULE_0__.Pagination, swiper_bundle__WEBPACK_IMPORTED_MODULE_0__.Lazy]);
var swipers = document.querySelectorAll(".slider"); //Находим все слайдеры в tab'ах

swipers.forEach(function (value) {
  var swiper = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](value, {
    slidesPerView: 3,
    spaceBetween: 30,
    lazy: true,
    init: false,
...
var totalSiteMainSlide = $('.SiteMain .swiper-wrapper .swiper-slide').length;
$('.tab_content:nth-child(1) .swiper__mobile .counter').append("<span class=\"counter__total\">\n/".concat(totalSiteMainSlide, "\n</span>"));
var totalSmmMainSlide = $('.SmmMain .swiper-wrapper .swiper-slide').length;
$('.tab_content:nth-child(2) .swiper__mobile .counter').append("<span class=\"counter__total\">\n/".concat(totalSmmMainSlide, "\n</span>"));
var totalContextMainSlide = $('.ContextMain .swiper-wrapper .swiper-slide').length;
$('.tab_content:nth-child(3) .swiper__mobile .counter').append("<span class=\"counter__total\">\n/".concat(totalContextMainSlide, "\n</span>"));
var totalBrandMainSlide = $('.BrandMain .swiper-wrapper .swiper-slide').length;
$('.tab_content:nth-child(4) .swiper__mobile .counter').append("<span class=\"counter__total\">\n/".concat(totalBrandMainSlide, "\n</span>"));
var totalPresentMainSlide = $('.PresentMain .swiper-wrapper .swiper-slide').length;
$('.tab_content:nth-child(5) .swiper__mobile .counter').append("<span class=\"counter__total\">\n/".concat(totalPresentMainSlide, "\n</span>"));
var totalPolyMainSlide = $('.PolyMain .swiper-wrapper .swiper-slide').length;
$('.tab_content:nth-child(6) .swiper__mobile .counter').append("<span class=\"counter__total\">\n/".concat(totalPolyMainSlide, "\n</span>"));
var brands = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"]('.mySwiper', {
  slidesPerView: 4,
  spaceBetween: 30,
  loop: true,
  autoplay: {
...
var totalSITESlide = $('.site .swiper-wrapper .swiper-slide').length;
var totalSMMSlide = $('.smms .swiper-wrapper .swiper-slide').length;
var totalCONTEXTSlide = $('.context__content .swiper-wrapper .swiper-slide').length;
var totalBRANDINGSlide = $('.branding .swiper-wrapper .swiper-slide').length;
var totalPRESENTATIONSlide = $('.presentation .swiper-wrapper .swiper-slide').length;
var totalPOLYGRAPHYSlide = $('.polygraphy .swiper-wrapper .swiper-slide').length; //==========

var site = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](".site", {
  simulateTouch: false,
  init: false,
  observer: true,
  observeParents: true,
...
site.on("slideChange afterInit init", function () {
  ...
  document.querySelector('.counter-site').innerHTML = "\n  <span class=\"counter__current\">\n  ".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "\n  </span>  \n  <span class=\"counter__total\">\n    <span>/ </span>").concat(totalSITESlide < 10 ? '0' + totalSITESlide : totalSITESlide, "\n  </span>");
});
var smm = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](".smms", {
  simulateTouch: false,
  init: false,
  observer: true,
  observeParents: true,
...
smm.on("slideChange afterInit init", function () {
  var currentSlide = this.activeIndex + 1;
  var totalSlide = this.slides.length;
  document.querySelector('.counter-smm').innerHTML = "\n  <span class=\"counter__current\">\n  ".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "\n  </span>  \n  <span class=\"counter__total\">\n    <span>/ </span>").concat(totalSMMSlide < 10 ? '0' + totalSMMSlide : totalSMMSlide, "\n  </span>");
});
var context__content = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](".context__content", {
  simulateTouch: false,
  init: false,
  observer: true,
  observeParents: true,
...
context__content.on("slideChange afterInit init", function () {
  var currentSlide = this.activeIndex + 1;
  var totalSlide = this.slides.length;
  document.querySelector('.counter-context').innerHTML = "\n  <span class=\"counter__current\">\n  ".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "\n  </span>  \n  <span class=\"counter__total\">\n    <span>/ </span>").concat(totalCONTEXTSlide < 10 ? '0' + totalCONTEXTSlide : totalCONTEXTSlide, "\n  </span>");
});
var branding = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](".branding", {
  simulateTouch: false,
  init: false,
  observer: true,
  observeParents: true,
...
branding.on("slideChange afterInit init", function () {
  var currentSlide = this.activeIndex + 1;
  var totalSlide = this.slides.length;
  document.querySelector('.counter-branding').innerHTML = "\n  <span class=\"counter__current\">\n  ".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "\n  </span>  \n  <span class=\"counter__total\">\n    <span>/ </span>").concat(totalBRANDINGSlide < 10 ? '0' + totalBRANDINGSlide : totalBRANDINGSlide, "\n  </span>");
});
var presentation = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](".presentation", {
  simulateTouch: false,
  init: false,
  autoHeight: true,
  observer: true,
...
presentation.on("slideChange afterInit init", function () {
  var currentSlide = this.activeIndex + 1;
  var totalSlide = this.slides.length;
  document.querySelector('.counter-presentation').innerHTML = "\n  <span class=\"counter__current\">\n  ".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "\n  </span>  \n  <span class=\"counter__total\">\n    <span>/ </span>").concat(totalPRESENTATIONSlide < 10 ? '0' + totalPRESENTATIONSlide : totalPRESENTATIONSlide, "\n  </span>");
});
var polygraphy = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"](".polygraphy", {
  simulateTouch: false,
  init: false,
  autoHeight: true,
  observer: true,
...
polygraphy.on("slideChange afterInit init", function () {
  var currentSlide = this.activeIndex + 1;
  var totalSlide = this.slides.length;
  document.querySelector('.counter-polygraphy').innerHTML = "\n  <span class=\"counter__current\">\n  ".concat(currentSlide < 10 ? '0' + currentSlide : currentSlide, "\n  </span>  \n  <span class=\"counter__total\">\n    <span>/ </span>").concat(totalPOLYGRAPHYSlide < 10 ? '0' + totalPOLYGRAPHYSlide : totalPOLYGRAPHYSlide, "\n  </span>");
});
...
$('.services__content').each(function () {
  ...
  tabTabs.each(function (i) {
    $(this).click(function () {
      ...
      $(tabItems[i]).find('.anim__item').not('.anim__active').addClass('anim__active');
      tabItems.not(tabItems[i]).removeClass('active').hide();
      /* animItemsServices.classList.add(`anim__active`) */
    });
  });
});
$('.form__switch__wrapper').each(function () {
  var acc = $(this).find('.quiz__switch__item'),
      i;
...
$('.quiz__switch__wrapper').each(function () {
  var acc = $(this).find('.quiz__switch__item'),
      i;
...
$('.tab_content .button .btn__arrow__stroke').click(function () {
  ...
  if ($(this).hasClass('btn--prev')) {
    if (tabsToggler.prev().length) {
      ...
      tabsContent.prev().find('.anim__item').not('.anim__active').addClass('anim__active');
    }
    ...
    if (tabsToggler.next().length) {
      ...
      tabsContent.next().find('.anim__item').not('.anim__active').addClass('anim__active');
    }
  }
});
...
$(function () {
  $('.form__wrapp').append("<div class=\"checkbox__default\">\n\t\t\t<input type=\"checkbox\" name=\"politics\" id=\"politics\" checked>\n\t\t\t<label for=\"politics\"><p>\u0412\u044B \u0441\u043E\u0433\u043B\u0430\u0448\u0430\u0435\u0442\u0435\u0441\u044C \u0441 <a data-policy=\"policy\">\u043F\u043E\u043B\u0438\u0442\u0438\u043A\u043E\u0439 \u043A\u043E\u043D\u0444\u0438\u0434\u0435\u043D\u0446\u0438\u0430\u043B\u044C\u043D\u043E\u0441\u0442\u0438</a></p></label>\n\t\t</div>");
  $('.price__form').append("<div class=\"checkbox__default\">\n\t\t\t<input type=\"checkbox\" name=\"politics\" id=\"politics\" checked>\n\t\t\t<label for=\"politics\"><p>\u0412\u044B \u0441\u043E\u0433\u043B\u0430\u0448\u0430\u0435\u0442\u0435\u0441\u044C \u0441 <a data-policy=\"policy\">\u043F\u043E\u043B\u0438\u0442\u0438\u043A\u043E\u0439 \u043A\u043E\u043D\u0444\u0438\u0434\u0435\u043D\u0446\u0438\u0430\u043B\u044C\u043D\u043E\u0441\u0442\u0438</a></p></label>\n\t\t</div>");
});

$('.btn__upper').on('click', function () {
...
$(function () {
  $(window).on('scroll', function () {
    if ($(this).scrollTop() > 100) {
      if ($('.btn__upper').is(':hidden')) {
        $('.btn__upper').css({
          opacity: 1
        }).fadeIn('slow');
      }
    } else {
      $('.btn__upper').stop(true, false).fadeOut('fast');
    }
  });
  $('.chaty-svg').addClass('customize');

  if (window.innerWidth <= 990) {
    document.querySelector('.chaty-channel .on-hover-text').textContent = 'Есть вопросы?';
    /* $('.on-hover-text').hide(); */
  }

  var policy = false;
  $('a[data-policy="policy"]').click(function () {
    if (policy == false) {
      $('.modal-policy').addClass('mp-open');
      $('.modal__wrapp--policy[data-policy="policy"]').css('display', 'flex');
      document.body.style.overflow = 'hidden';
      policy = true;
    }
  });
  $('.modal__close--policy').click(function () {
    if (policy == true) {
      $('.modal-policy').removeClass('mp-open');
      $('.modal__wrapp--policy[data-policy="policy"]').css('display', 'none');
...
document.addEventListener('wpcf7mailsent', function (event) {
  ...
  if ('115' == event.detail.contactFormId) {
    //оставить заявку и получить кп
    ym(90018441, 'reachGoal', 'zayavka');
  } else if ('113' == event.detail.contactFormId) {
    //обсудить проблему
    ym(90018441, 'reachGoal', 'problem');
  } else if ('187' == event.detail.contactFormId) {
    //аудит
    ym(90018441, 'reachGoal', 'audit');
  } else if ('228' == event.detail.contactFormId) {
    //контакты
    ym(90018441, 'reachGoal', 'zakaz_zvonok');
  } else if ('119' == event.detail.contactFormId) {
    if (document.querySelector('input.service__input').value == "Сайты Start" || document.querySelector('input.service__input').value == "Сайты PRO" || document.querySelector('input.service__input').value == "Редизайн сайта") {
      ym(90018441, 'reachGoal', 'zyyavka_site');
    } else if (document.querySelector('input.service__input').value == "SMM") {
      ym(90018441, 'reachGoal', 'zayvka_smm');
    } else if (document.querySelector('input.service__input').value == "Брендинг и фирменный стиль") {
      ym(90018441, 'reachGoal', 'zayavka_brend');
    } else if (document.querySelector('input.service__input').value == "CRM") {
      ym(90018441, 'reachGoal', 'zayvka_crm');
    }
  }
}, false);
$(document).ready(function () {
